...
PHP_FUNCTION(curl_persistent_share_init)
{
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(share_opts), entry) {
		...
		persistent_id += 1 << zval_get_long_ex(entry, true);
	...
	if (persistent_id) {
		zval *persisted = zend_hash_index_find(&CURL_G(persistent_share_handles), persistent_id);
...
{
	php_error_docref(NULL, E_WARNING, "in module global free: %p", data);
...
static zend_object *curl_share_create_object(zend_class_entry *class_type) {
	php_error_docref(NULL, E_WARNING, "in create: %p", class_type);
...
{
	php_error_docref(NULL, E_WARNING, "in free: %p", object);
...
void curl_persistent_share_register_handlers(void) {
	...
	curl_persistent_share_handlers.get_constructor = curl_share_get_constructor; // TODO(enorris)
